...
bool QuaZip::getCurrentFileInfo(QuaZipFileInfo *info)const
{
  ...
  if((fakeThis->p->zipError=unzGetCurrentFileInfo(p->unzFile_f, NULL,
    ...
    return false;
  info->versionCreated=static_cast<quint16>(info_z.version);
  info->versionNeeded=static_cast<quint16>(info_z.version_needed);
  info->flags=static_cast<quint16>(info_z.flag);
  info->method=static_cast<quint16>(info_z.compression_method);
  info->crc=info_z.crc;
  info->compressedSize=info_z.compressed_size;
  info->uncompressedSize=info_z.uncompressed_size;
  info->diskNumberStart=static_cast<quint16>(info_z.disk_num_start);
  info->internalAttr=static_cast<quint16>(info_z.internal_fa);
