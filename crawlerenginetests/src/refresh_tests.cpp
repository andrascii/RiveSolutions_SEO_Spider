...
TEST(RefreshTests, OldDuplicatesMustBeRemovedAfterRefreshDifferentCanonicals)
{
	...
	const auto testFunction = [cl = env.crawler()]()
	{
		...
		const auto onPageRefreshed = [&]
		{
			...
			EXPECT_EQ(true, std::all_of(titleDuplicatesAfterRefresh.begin(), titleDuplicatesAfterRefresh.end(), firstTimeDuplicateTitle));
			EXPECT_EQ(true, std::all_of(metaDescriptionDuplicatesAfterRefresh.begin(), metaDescriptionDuplicatesAfterRefresh.end(), firstTimeDuplicateMetaDescription));
		};
