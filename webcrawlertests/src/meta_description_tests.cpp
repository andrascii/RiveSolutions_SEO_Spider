...
TEST(MetaDescriptionTests, EmptyMetaDescription)
{
	...
	{
		auto pages = cl->waitForParsedPageReceived(WebCrawler::EmptyMetaDescriptionUrlStorageType, 2, 10, "Waiting for 2 empty meta description pages");
...
TEST(MetaDescriptionTests, TooLongMetaDescription)
{
	...
	WebCrawler::CrawlerOptions options = TestEnvironment::defaultOptions(QUrl("http://meta-desc.com/too-long-meta.html"));
	options.maxDescriptionLength = 10;
	TestEnvironment env(options);
	env.runTest([cl = env.crawler()]()
	{
		auto pages = cl->waitForParsedPageReceived(WebCrawler::VeryLongMetaDescriptionUrlStorageType, 2, 10, "Waiting for 2 too long meta desc pages");
		EXPECT_EQ(2, pages.size());
...
TEST(MetaDescriptionTests, TooShortMetaDescription)
{
	...
	WebCrawler::CrawlerOptions options = TestEnvironment::defaultOptions(QUrl("http://meta-desc.com/too-short-meta.html"));
	options.minDescriptionLength = 50;
	options.maxDescriptionLength = 100;
	TestEnvironment env(options);
	env.runTest([cl = env.crawler()]()
	{
		auto pages = cl->waitForParsedPageReceived(WebCrawler::VeryShortMetaDescriptionUrlStorageType, 2, 10, "Waiting for 2 too short meta desc pages");
		EXPECT_EQ(2, pages.size());
...
TEST(MetaDescriptionTests, DuplicatedMetaDescriptions)
{
	...
	WebCrawler::CrawlerOptions options = TestEnvironment::defaultOptions(QUrl("http://meta-desc.com/duplicated-meta-1.html"));
	TestEnvironment env(options);
	env.runTest([cl = env.crawler()]()
	{
		auto pages = cl->waitForParsedPageReceived(WebCrawler::DuplicatedMetaDescriptionUrlStorageType, 2, 10, "Waiting for 2 duplicated meta desc pages");
		EXPECT_EQ(2, pages.size());
...
TEST(MetaDescriptionTests, SeveralMetaDescriptions)
{
	...
	env.runTest([cl = env.crawler()]()
	{
		auto pages = cl->waitForParsedPageReceived(WebCrawler::SeveralMetaDescriptionUrlStorageType, 2, 10, "Waiting for 2 several meta desc pages");
		EXPECT_EQ(2, pages.size());
